...
class KGLatentNetV3(nn.Module):
    VARIANTS = (
        "v3_small_strong_anchor",
        "v3_summary_anchor",
        "v3_regularized_fusion",
    )
    ...
    def __init__(
        ...
        summary_dim: int = 32,
        dropout: float = 0.5,
        model_variant: str = "v3_small_strong_anchor",
        ...
        if model_variant == "v3_small_strong_anchor":
            self.delta_raw_head = nn.Sequential(
        ...
        elif model_variant == "v3_summary_anchor":
            ...
            self.delta_latent_head = nn.Sequential(
                nn.Linear(latent_dim + hidden_dim, hidden_dim // 2 if hidden_dim > 4 else hidden_dim),
                nn.ReLU(),
                nn.Linear(hidden_dim // 2 if hidden_dim > 4 else hidden_dim, 1),
            )
            self.mix_alpha = nn.Parameter(torch.tensor(-2.0))

        elif model_variant == "v3_regularized_fusion":
            self.delta_latent_head = nn.Sequential(
            ...
            self.anchor_scale_gate = nn.Sequential(
                nn.Linear(latent_dim + summary_dim + hidden_dim, hidden_dim // 2 if hidden_dim > 4 else hidden_dim),
                nn.ReLU(),
                nn.Linear(hidden_dim // 2 if hidden_dim > 4 else hidden_dim, 1),
                nn.Sigmoid(),
            )
    ...
    def forward(
        ...
        gate_entropy = torch.tensor(0.0, device=baseline.device)
        gate_weights = torch.zeros(baseline.shape[0], 3, device=baseline.device)
        delta_latent = torch.zeros_like(baseline)
        delta_summary = torch.zeros_like(baseline)
        delta_treatment = torch.zeros_like(baseline)

        if self.model_variant == "v3_small_strong_anchor":
            delta_input = torch.cat([z_final, summary_hidden, baseline_hidden], dim=-1)
            delta_raw = self.delta_raw_head(delta_input).squeeze(-1)
            scale = self.scale_gate(baseline_hidden).squeeze(-1)
            delta_pred = scale * delta_raw

        elif self.model_variant == "v3_summary_anchor":
            delta_summary = self.delta_summary_head(
            ...
            delta_pred = (1.0 - alpha) * delta_summary + alpha * delta_latent
            gate_weights[:, 0] = alpha
            gate_weights[:, 1] = 1.0 - alpha

        elif self.model_variant == "v3_regularized_fusion":
            delta_latent = self.delta_latent_head(
                torch.cat([z_final, baseline_hidden], dim=-1)
            ).squeeze(-1)
            ...
            scale = self.anchor_scale_gate(gate_input).squeeze(-1)
            delta_pred = scale * delta_raw
    ...
    def compute_loss(
        ...
        stage_loss_weight: str = "none",
    ) -> dict[str, torch.Tensor]:
        y_true = batch["endpoint_tbr_y"].squeeze(-1)
        baseline = batch["baseline_tbr_b"].squeeze(-1)
        delta_true = y_true - baseline

        if stage_loss_weight == "mild_long_term_weight" and "endpoint_window" in batch:
            weight_map = {6: 1.0, 12: 1.0, 18: 1.1, 24: 1.1}
            windows = batch["endpoint_window"]
            sample_weights = torch.tensor(
                [weight_map.get(int(w.item()), 1.0) for w in windows],
                device=y_true.device, dtype=torch.float32,
            )
            per_sample = torch.abs(outputs["y_pred"] - y_true)
            l_endpoint = (per_sample * sample_weights).mean()
        else:
        ...
        l_gate_entropy = torch.tensor(0.0, device=total.device)
        if lambda_gate_entropy > 0:
            l_gate_entropy = outputs["gate_entropy"].mean()
            total = total + lambda_gate_entropy * l_gate_entropy
