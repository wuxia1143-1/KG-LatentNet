...
class ResidualTensorDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        tensor_keys = [
            "static_features", "dynamic_features", "dynamic_mask",
            "delta_time", "treatment_features", "baseline_tbr_b", "endpoint_tbr_y",
        ]
        tensors = {
...
def setup_logger(project_root: Path, fold: int, log_name: str | None = None) -> logging.Logger:
    ...
    name = log_name or f"kg_latentnet_v3_fold{fold}"
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            loss_dict = KGLatentNetV3.compute_loss(
                ...
                y_range=y_range, lambda_range=lambda_range,
...
def train_one_fold(
    ...
    model_variant = str(config.get("model_variant", "v3_small_strong_anchor"))
    lambda_delta = float(config.get("lambda_delta", 0.5))
    lambda_anchor = float(config.get("lambda_anchor", 0.1))
    ...
    lambda_gate_entropy = float(config.get("lambda_gate_entropy", 0.0))
    ...
    stage_loss_weight = str(config.get("stage_loss_weight", "none"))

    logger.info("=== Fold %d Training ===", fold)
    logger.info("model_variant=%s hidden_dim=%d latent_dim=%d summary_dim=%d dropout=%.2f",
                model_variant, hidden_dim, latent_dim, summary_dim, dropout)
    logger.info("lr=%.6f wd=%.6f loss=%s huber_delta=%.4f gradient_clip=%.1f", lr, wd, loss_type, huber_delta, gradient_clip)
    logger.info("lambda_delta=%.4f lambda_anchor=%.4f lambda_prior=%.4f lambda_smooth=%.4f",
                lambda_delta, lambda_anchor, lambda_prior, lambda_smooth)
    logger.info("lambda_disentangle=%.4f lambda_gate_entropy=%.4f stage_loss_weight=%s",
                lambda_disentangle, lambda_gate_entropy, stage_loss_weight)
    ...
    model = KGLatentNetV3(
        ...
        dropout=dropout, model_variant=model_variant, huber_delta=huber_delta,
    ).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        val_loss, val_preds, _ = evaluate_model(
            ...
            lambda_smooth, lambda_disentangle, lambda_gate_entropy,
            y_range, lambda_range, stage_loss_weight, fold,
        )
...
def smoke_test(
    ...
    model_variant = config.get("model_variant", "v3_small_strong_anchor")

    logger = setup_logger(project_root, fold, f"kg_latentnet_v3_{model_variant}_smoke_test")
    ...
    if leaked:
        logger.warning("Leakage detected: %s", leaked)
...
def main(argv: list[str] | None = None) -> dict[str, Any]:
    parser = argparse.ArgumentParser(description="KG-LatentNet V3 training / smoke test")
    parser.add_argument("--project-root", default=str(PROJECT_ROOT))
    parser.add_argument("--fold", type=int, default=0)
    parser.add_argument("--mode", choices=["smoke", "train"], default="smoke")
    parser.add_argument("--config", type=str, default="")
    parser.add_argument("--epochs", type=int, default=5)
    parser.add_argument("--model-variant", type=str, default="v3_small_strong_anchor")
    args = parser.parse_args(argv)

    project_root = Path(args.project_root).resolve()

    config = {
        "model_variant": "v3_small_strong_anchor",
        "hidden_dim": 16,
        "latent_dim": 8,
        "summary_dim": 32,
        "dropout": 0.5,
