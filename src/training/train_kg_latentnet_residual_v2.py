...
from src.models.kg_latentnet_residual_v2 import KGLatentNetResidualV2
...
def setup_logger(project_root: Path, fold: int, log_name: str | None = None) -> logging.Logger:
    ...
    name = log_name or f"kg_latentnet_residual_v2_fold{fold}"
...
def train_one_fold(
    ...
    variant_mode = str(config.get("variant_mode", "residual_anchor_v1"))
    huber_delta = float(config.get("huber_delta", 0.1))
    correction_scale = float(config.get("correction_scale", 0.5))
    lambda_correction_magnitude = float(config.get("lambda_correction_magnitude", 0.0))

    logger.info("=== Fold %d Training ===", fold)
    logger.info("variant_mode=%s target_mode=%s hidden_dim=%d latent_dim=%d dropout=%.2f", variant_mode, target_mode, hidden_dim, latent_dim, dropout)
    ...
    logger.info("correction_scale=%.4f lambda_correction_magnitude=%.4f", correction_scale, lambda_correction_magnitude)
    ...
    pred_fields = ["patient_id", "fold", "endpoint_window", "baseline_tbr_b", "y_true", "delta_true", "delta_pred", "y_pred", "absolute_error", "variant_mode"]
...
def smoke_test(
    ...
    variant_mode = config.get("variant_mode", "residual_anchor_v1")

    logger = setup_logger(project_root, fold, f"kg_latentnet_residual_v2_{variant_mode}_smoke_test")
    ...
    pred_path = project_root / "results" / "predictions" / "full_5fold" / f"kg_latentnet_residual_v2_{variant_mode}_fold0_smoke_predictions.csv"
    write_csv(pred_path, val_preds, result["pred_fields"])
    logger.info("Saved predictions: %s", pred_path)

    metrics_rows = [{"epoch": "smoke", "variant_mode": variant_mode, "val_mae": result["final_val_mae"], "val_rmse": result["final_val_rmse"], "val_r2": result["final_val_r2"]}]
    metrics_path = project_root / "results" / "tables" / "full_5fold" / f"kg_latentnet_residual_v2_{variant_mode}_smoke_metrics.csv"
    write_csv(metrics_path, metrics_rows, ["epoch", "variant_mode", "val_mae", "val_rmse", "val_r2"])
    logger.info("Saved metrics: %s", metrics_path)

    latent_path = project_root / "results" / "latent" / "full_5fold" / f"kg_latentnet_residual_v2_{variant_mode}_fold0_latent_states_smoke.pkl"
    ...
    contrib_path = project_root / "results" / "latent" / "full_5fold" / f"kg_latentnet_residual_v2_{variant_mode}_fold0_contributions_smoke.pkl"
    ...
    logger.info("variant_mode=%s smoke_val_mae=%.6f", variant_mode, result["final_val_mae"])

    return result


def main(argv: list[str] | None = None) -> dict[str, Any]:
    parser = argparse.ArgumentParser(description="KG-LatentNet-Residual V2 training / smoke test")
    parser.add_argument("--project-root", default=str(PROJECT_ROOT))
    parser.add_argument("--fold", type=int, default=0)
    parser.add_argument("--mode", choices=["smoke", "train"], default="smoke")
    parser.add_argument("--config", type=str, default="")
    parser.add_argument("--epochs", type=int, default=5)
    parser.add_argument("--variant-mode", type=str, default="residual_anchor_v1")
    ...
    if args.variant_mode:
        config["variant_mode"] = args.variant_mode
