...
OUT = ROOT / "results" / "honest_paper_repro_kg_v8_kg_only_meta_prior"
TABLES = OUT / "tables"
FIGURES = OUT / "figures"
...
def matrix(df: pd.DataFrame, cols: list[str]) -> np.ndarray:
    ...
    summary = np.column_stack([preds.mean(axis=1), preds.std(axis=1), preds.min(axis=1), preds.max(axis=1)])
...
def make_figures(conf: pd.DataFrame, eval_df: pd.DataFrame) -> None:
    ...
    ax.set_title("V8 KG-only meta-prior readouts")
    ax.legend(fontsize=6, ncol=2)
    fig.tight_layout()
    fig.savefig(FIGURES / "figure_v8_kg_only_mae.png", dpi=180)
    plt.close(fig)
...
def main() -> None:
    ...
    key_map.to_csv(TABLES / "kg_v8_selected_kg_readout_keys.csv", index=False, encoding="utf-8-sig")
    val = build_validation_wide(keys, key_map)
    test = build_test_wide(keys)
    val.to_csv(TABLES / "kg_v8_validation_wide_predictions.csv", index=False, encoding="utf-8-sig")
    test.to_csv(TABLES / "kg_v8_test_wide_predictions.csv", index=False, encoding="utf-8-sig")
    ...
    for idx, spec in enumerate(candidate_specs):
        ...
        if (idx + 1) % 40 == 0:
            print(f"[v8] evaluated {idx + 1}/{len(candidate_specs)}", flush=True)
    rank = pd.DataFrame(rank_rows)
    ok = rank[rank["status"].eq("ok")].copy()
    ok["kg_only_validation_objective"] = (
        ok["nested_val_mae"]
        + 0.35 * ok["nested_val_long_mae"].fillna(ok["nested_val_mae"])
        + 0.10 * ok["nested_val_24_mae"].fillna(ok["nested_val_mae"])
        - 0.015 * ok["nested_val_24_rho"].fillna(0)
    )
    ...
    rank.to_csv(TABLES / "kg_v8_nested_validation_candidate_rank.csv", index=False, encoding="utf-8-sig")
    preds = pd.concat(pred_frames, ignore_index=True)
    eval_df = pd.concat(eval_frames, ignore_index=True)
    preds.to_csv(PRED / "kg_v8_all_candidate_predictions.csv", index=False, encoding="utf-8-sig")
    eval_df.to_csv(TABLES / "kg_v8_external_audit_by_scope.csv", index=False, encoding="utf-8-sig")
    conf = conformance(eval_df, rank)
    conf.to_csv(TABLES / "kg_v8_limit_removal_audit.csv", index=False, encoding="utf-8-sig")
    make_figures(conf, eval_df)
    provenance = {
        "created_by": "honest_kg_latentnet_v8_kg_only_meta_prior.py",
        "integrity_note": "V8 is KG-only: all trainable/readout inputs are KG-LatentNet candidate predictions. RF is used only as an external audit baseline after predictions are fixed.",
